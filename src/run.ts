...
function setGithubRunId() {
  const runId = process.env.GITHUB_RUN_ID;
  process.env.COVERALLS_SERVICE_JOB_ID = runId;
  return runId;
}

function handleParallelFinished() {
  return new Promise((res, rej) => {
    const runId = setGithubRunId();
...
export async function run() {
  try {
    ...
    setCoverallsEndpoint();

    if(core.getInput('parallel-finished') != '') {
      try {
        await handleParallelFinished();
      } catch(e) {
        return 1;
      }
      return 0;
