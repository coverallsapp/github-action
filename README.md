...
| --------------------- | ----------- | ----------- |
| `github-token`        | _required_ | Default: `${{ github.token }}`. Can be also used this way: `github-token: ${{ secrets.GITHUB_TOKEN }}`; Coveralls uses this token to verify the appropriate repo and create any new checks based on your coverage results. This variable is built into Github Actions, so __should not be added to your secrets store__. [More Info](https://help.github.com/en/actions/configuring-and-managing-workflows/authenticating-with-the-github_token)|
